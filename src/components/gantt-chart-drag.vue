    ...
    <div class="gantt-app-view"
      ...
      <div class="container__1PWP"
        ...
        <div ref="ganttBody"
             ...
             :width="gantW"
             :height="gantH">
          <div class="scroll-indicator__3aij"
          ...
          <header>
            <table-header ref="tableHeader"
                          ...
                          @onAllRowOpen="onAllRowOpen"></table-header>
            <div ref="timeAxisRender"
                 @wheel.prevent="wheel"
                 class="time-axis__3meF"
                 :style="`left: ${tableWidth}px; width: ${viewWidth}px;`">
              <div class="render-chunk__28qu"
                   :style="`transform: translateX(-${translateX}px;`">
                <div v-for="(item, key) in getMajorList()"
                     :key="key"
                     class="major__2rd6"
                     :style="`width: ${item.width}px; left: ${item.left}px;`">
                  <div class="label__RLd1">{{item.label}}</div>
                </div>
                <div v-for="(dayItem) in getMinorList()"
                     class="minor__11Xd"
                     :class="{
                  ...
                  <div v-if="dayItem.isHighlight && viewTypeObj.key === 'day'"
                       class="highlight-bg__1mPp"></div>
                </div>
              </div>
            </div>
          </header>
          <main ref="gantMainEl">
            <div class="selection-indicator__3rr6"
                 v-show="showSelectionIndicator"
                 :style="`display: none; height: 28px; top: ${selectionIndicatorTop}px;`"></div>
            <table-body ref="tableBody"
                        ...
                        @onRowOpen="onRowOpen"></table-body>
            <div ref="chartView"
                 @wheel.prevent="wheel"
              ...
              <svg class="chart-svg-renderer__7ors"
                ...
                <template v-for="(item) in getMinorList()">
                  <g v-if="item.isWeek"
                    ...
                    <rect fill="#F7F7F7"
                          ...
                          y="0"
                          :width="item.width"
                          :height="svgViewH"></rect>
                    <rect v-if="item.isHighlight"
                          fill="#FFA941"
                          opacity="0.3"
                          stroke-width="0"
                          :x="item.highlightX"
                          y="0"
                          :width="item.highlightW"
                          :height="svgViewH"></rect>
                  </g>
                  <g v-else
                     :key="item.key"
                ...
                <g v-if="showDragToolShadow"
                   fill="rgba(204, 236, 255, 0.3)"
                   stroke="#87D2FF">
                  <path v-show="shadowGestBarLeft"
                        :d="`M${shadowGestBarLeft},0 L${shadowGestBarLeft},${svgViewH}`"></path>
                  <rect :x="dragToolShadowX"
                        y="0"
                        :width="dragToolShadowW"
                        :height="svgViewH"
                        stroke-width="0"></rect>
                  <path v-show="shadowGestBarRight"
                        :d="`M${shadowGestBarRight},0 L${shadowGestBarRight},${svgViewH}`"></path>
                </g>
              </svg>
              <div class="render-chunk__22Ez"
                   :style="`height: ${svgViewH}px; transform:translateX(-${translateX}px;`">
                <template v-for="(bar, index) in barList">
                  <task-bar-thumb v-if="getshowTaskBar(bar.width, bar.translateX, translateX) && !bar.invalidDateRange"
                                  :key="index"
                                  :label="bar.label"
                                  :color="bar.color"
                                  ...
                                  :width="bar.width"
                                  @timeTranslateLocation="locaTimeTranslate"></task-bar-thumb>
                  <task-bar v-else
                            :key="index"
                            :label="bar.label"
                            :color="bar.color"
                            ...
                            @gestureBarPressup="(event) => shadowGestureBarPressup(event, bar)"></task-bar>
                </template>
                <template v-for="(bar, key) in barList.filter(item => item.invalidDateRange)">
                  <invalid-task-bar :key="barList.length + key"
          ...
          <divider-split @showTable="showTable"
                         :layIsHandleOver="layIsHandleOver"
                         :left="tableWidth"></divider-split>
          <time-indicator :guestureGrantBodyMove="guestureGrantBodyMove"
                          :viewTranslateX="translateX"
                          :tableWidth="tableWidth"
                          :viewWidth="viewWidth"
                          :pxUnitAmp="pxUnitAmp"
                          @timeTranslateLocation="locaTimeTranslate"></time-indicator>
          <time-axis-scale-select v-model="viewTypeObj"
                                  :guestureGrantBodyMove="guestureGrantBodyMove"
                                  :viewWidth="viewWidth"
                                  :view-type-list="viewTypeList"
                                  :defaultValue="viewTypeObj"></time-axis-scale-select>
        </div>
      </div>
    </div>
  </div>
...
import invalidTaskBar from './invalid-task-bar.vue'
import TaskBarThumb from './task-bar-thumb.vue'
...
import SvgSymbol from './svg-symbol.vue'
import DividerSplit from './divider-split.vue'
...
const aTick =
  'function' === typeof requestAnimationFrame ? requestAnimationFrame : (e) => setTimeout(() => e(Date.now()), 1e3 / 60)
