<template>
  <div @mouseenter="triggerEnter"
       @mouseleave="triggerLeave"
       @mousemove="triggerMove"
       @mousedown="triggerDown">
    <div class="task-row-trigger__1Xhy"
         ref="trigger"
         :style="`left: ${translateX}px; transform: translateY(${top - ((rowH - barH) / 2)}px);`"></div>
    <div v-if="isShow"
         class="block__1ibh"
         aria-haspopup="true"
         aria-expanded="false"
      ...
      <div class="date__7am6"
           :style="`right: ${Math.ceil(barWidth + 6)}px;`">{{dateTextFormat(barLeft)}}</div>
      <div class="date__7am6"
           :style="`left: ${Math.ceil(barWidth + 6)}px;`">{{dateTextFormat(barLeft + barWidth)}}</div>
    </div>
  </div>
</template>
<script>
