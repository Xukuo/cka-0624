    ...
    <div ref="bodyView"
      ...
      <template v-for="(barInfo, index) in rowDataList">
        <div :key="index"
          ...
          <div class="cell__3xqP resizable__3OIa"
               :class="columns[0]._isHandleOver ? 'resize-active__1t-e' : 'resize-default__2DLj'"
               :style="`width: ${columns[0].width}px;`">
            <div class="row-before__3blm"
                 style="padding-left: 56px;">
              <div class="row-index__3xNX"
                   title="1"
                   style="width: 24px;">{{index + 1}}</div>
              <div class="row-indentation__2dHs"
                   :style="`background-size: ${indent}px; width: ${getIndent(barInfo._depth)}px;`"></div>
              <div class="row-menu-and-toggler___F4q">
                <div class="row-menu__QRiG">
                  <task-menu :barInfo="barInfo"
                             ...
                             @moveLeftTask="moveLeftTask"></task-menu>
                </div>
                <div v-if="getShowTrigger(barInfo)"
                     @click="rowTrigger(barInfo)"
                     class="row-toggler__3rTS">
                  <div class="body-row-toggler__wvWq"
                       :class="{collapsed: barInfo._collapsed}">
            ...
            <div class="body-cell__OUd5 content">
              <div v-if="getContentReadOnly(barInfo)"
                   class="text__aNJc"
            ...
            <div class="handle__cGEN right"
                 ...
                 @mouseout="handleMouseOver(columns[0], false)"></div>
          </div>
          <div class="cell__3xqP resizable__3OIa"
            ...
            <div class="body-cell__OUd5 executor__1eK3 hasPermission empty">
              ...
              <span class="text">{{barInfo.task.executor ? barInfo.task.executor : '待认领'}}</span>
            </div>
            <div class="handle__cGEN right"
                 data-role="handle"
                 @mousedown="(event) => dispatchGesture('mouseDown', event)"
                 @mouseover="handleMouseOver(columns[1], true)"
                 @mouseout="handleMouseOver(columns[1], false)"></div>
          </div>
          <div class="cell__3xqP resizable__3OIa"
            ...
            <div class="handle__cGEN right"
                 ...
                 @mouseout="handleMouseOver(columns[2], false)"></div>
          </div>
          <div class="cell__3xqP resizable__3OIa"
            ...
            <div class="handle__cGEN right"
                 ...
                 @mouseout="handleMouseOver(columns[3], false)"></div>
          </div>
        </div>
      </template>
      <div class="row__29JV"
           :style="`top: ${(rowDataList.length * rowHeight + 4)}px; height: 28px;`">
        <div class="cell__3xqP resize-default__2DLj resizable__3OIa"
             :style="`width: ${columns[0].width}px;`">
          <div class="row-before__3blm"
               style="padding-left: 56px;">
            <div class="row-index__3xNX"></div>
            <div class="row-indentation__2dHs"
                 style="background-size: 38px; width: 0px;"></div>
            <div class="row-menu-and-toggler___F4q"></div>
          </div>
          <div class="foot__1A82 gantt-app-outline-foot">
            <button class="textbtn-color-blue__361l"
                    type="button"
      ...
      <div v-show="rowDataList[handleIndex]"
           class="mask__3CAH"
           @mousemove="gestureMove"></div>
      <div v-if="rowDataList[handleIndex]"
           class="drag-indicator__1C3_"
           :style="`left: ${handleLeft}px; top: ${handleTop}px;`"></div>
    </div>
  </div>
</template>
<script>
...
const Input = {
  ...
  methods: {
    ...
    keydown($event) {
      // enter 按键
      if (!(!$event.type.indexOf('key') && this._k($event.keyCode, 'enter', 13, $event.key, 'Enter'))) {
        return this.onKeydownEnter($event)
      }

      // shift + tab 组合按键
      if (!(!$event.type.indexOf('key') && this._k($event.keyCode, 'tab', 9, $event.key, 'Tab')) && $event.shiftKey) {
        return this.onKeyShiftTab($event)
      }

      // Tab 按键
      if (!(!$event.type.indexOf('key') && this._k($event.keyCode, 'tab', 9, $event.key, 'Tab'))) {
...
export default {
  ...
  methods: {
    ...
    moveRightTask(barInfo) {
      ...
      if (!preSibTask.children) preSibTask.children = []
