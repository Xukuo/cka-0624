<template>
  <div class="time-axis-scale-select__3fTI"
       v-clickoutside="hide">
    <div class="trigger__3NoY"
         @click="show">
      <div class="text__2O4S">{{value.label}}视图</div>
      <span class="icon next-icon icon-triangle-down-s next-medium"></span>
    </div>
    <div class="shadow__3n0C"
         :class="{scrolling__2c3k: guestureGrantBodyMove}"></div>
    <div v-if="!isHide"
         class="next-overlay-wrapper opened">
      <div class="next-loading next-loading-inline next-overlay-inner next-select-loading next-select-single-menu"
           aria-hidden="false"
           :style="`position: absolute; right: 15px; top: ${top}px;`">
        <div class="next-loading-wrap">
          <ul role="listbox"
              class="next-menu next-ver next-select-menu"
              aria-multiselectable="false">
            <li v-for="(item, key) in viewTypeList"
                :key="key"
                role="option"
                tabindex="-1"
                @click="select(item)"
              ...
              <div class="next-menu-item-inner">
                <i :class="{'next-menu-icon-selected': isSelected(item.key)}"
                   class="next-icon next-icon-tick next-medium">
                  <svg v-if="isSelected(item.key)"
                       id="at-tick"
                ...
                <span class="next-menu-item-text"
                      aria-selected="true">{{item.label}}</span>
              </div>
            </li>
          </ul>
        </div>
